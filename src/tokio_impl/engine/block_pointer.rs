...
impl BlockPropsPointer {
    ...
    pub(super) fn new(block: &mut BlockPropsType) -> Self {
        let block_props_ptr = block as *mut BlockPropsType;

        let ptr_ref = &block_props_ptr as *const *mut BlockPropsType;
    ...
    pub(super) fn get(&self) -> Option<*mut BlockPropsType> {
        if self.fat_pointer == [0; 2] {
            None
        } else {
            let ptr = {
                let pointer_parts: *const [usize; 2] = &self.fat_pointer;
                let ptr_ref = pointer_parts as *const *mut BlockPropsType;
                unsafe { *ptr_ref }
            };
